  ...
  return (
    <header 
      ...
      className={`sticky top-0 z-50 transition-all duration-200 ${
        isScrolled 
          ? 'bg-[#FBF9F5]/98 backdrop-blur-[18px] shadow-md border-b border-[#E6E2D8]' 
          : 'bg-[#FBF9F5] border-b border-[#E6E2D8]/80'
      }`}
    >
      {/* TIER 1: Institution Brand & Header Utilities */}
      <div className="border-b border-[#E6E2D8]/70">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2 sm:py-2.5 flex items-center justify-between gap-2 sm:gap-3">
        
          {/* Left: Logo Block (flexibly constrained to prevent shoving login button off-screen) */}
          <div 
          ...
          >
            ...
            <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-[#0D2440] text-[#C89B3C] flex items-center justify-center font-serif font-black text-base sm:text-xl tracking-wider shadow-md group-hover:scale-[1.03] transition-transform border border-[#C89B3C]/30 flex-shrink-0">
              BEC
            </div>
            <div className="flex flex-col text-left min-w-0 overflow-hidden">
              <span className="text-[9px] sm:text-[11px] font-semibold uppercase tracking-wider text-[#5C6B7A] leading-tight truncate">
                B.V.V. Sangha's
              </span>
              <span className="font-heading font-bold text-xs xs:text-sm sm:text-base 2xl:text-lg text-[#0D2440] leading-tight group-hover:text-[#C89B3C] transition-colors truncate">
                Basaveshwar Engg. College
                <span className="hidden md:inline"> (Autonomous)</span>
              </span>
              <span className="text-[9px] sm:text-[11px] font-mono font-medium text-[#C89B3C] tracking-wide leading-tight truncate">
                Bagalkote · ESTD 1963<span className="hidden sm:inline"> · Autonomous</span>
              </span>
            </div>
          </div>

          {/* Right End: Quick Badges, Login Area & Mobile Toggle */}
          <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0 ml-auto relative z-30">
            {/* Accreditation badge (desktop) */}
            <div className="hidden lg:flex flex-col items-end text-right border-r border-[#E6E2D8] pr-3 text-[11px]">
              <span className="font-mono text-[#0D2440] font-bold">Affiliated to VTU, Belagavi</span>
              <span className="text-[#5C6B7A]">NAAC 'A' Grade · NBA Accredited</span>
            </div>
            ...
            {userSession ? (
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-1 sm:py-1.5 text-xs font-semibold rounded-full bg-[#0D2440] text-white hover:bg-[#16365C] transition-all shadow-sm max-w-[130px] sm:max-w-none truncate"
                >
                  <User className="w-3.5 h-3.5 text-[#C89B3C] flex-shrink-0" />
                  <span className="hidden xs:inline font-mono truncate">{userSession.identifier}</span>
                  <span className="capitalize text-[#C89B3C] truncate">({userSession.role})</span>
                </button>
                <button
                  id="header-logout-btn"
                  onClick={onLogout}
                  title="Sign out"
                  className="p-1.5 text-[#5C6B7A] hover:text-[#0D2440] hover:bg-[#0D2440]/10 rounded-full transition-colors flex-shrink-0"
                >
              ...
              <div className="relative" ref={loginRef}>
                <button
                  ...
                  className="flex items-center gap-1 sm:gap-1.5 rounded-full px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold border-[1.5px] border-[#0D2440] text-[#0D2440] hover:bg-[#0D2440] hover:text-white transition-all duration-200 shadow-sm whitespace-nowrap"
                ...
                {/* Backdrop on mobile for touch outside dismissal */}
                {loginDropdownOpen && (
                  <div 
                ...
                {/* Login Dropdown - constrained within viewport on all screens */}
                {loginDropdownOpen && (
                  <div 
                    id="header-login-dropdown"
                    className="fixed right-2.5 top-[56px] sm:absolute sm:right-0 sm:top-full sm:mt-2 w-[calc(100vw-20px)] sm:w-64 max-w-xs bg-white rounded-2xl shadow-[0_16px_36px_-6px_rgba(10,22,40,0.22)] border border-[#E6E2D8] py-2 z-50 animate-in fade-in zoom-in-95 duration-150"
                  >
                    ...
                    >
                      <div className="w-7 h-7 rounded-lg bg-[#0D2440]/10 text-[#0D2440] flex items-center justify-center group-hover:bg-[#0D2440] group-hover:text-white transition-colors flex-shrink-0">
                        <GraduationCap className="w-4 h-4" />
                      </div>
                      <div className="min-w-0">
                        <div className="font-semibold text-[#0D2440] truncate">Student Login</div>
                        <div className="text-[11px] text-[#5C6B7A] truncate">USN & Results / Attendance</div>
                      </div>
                    </button>
                    ...
                    <button
                      ...
                      className="w-full text-left px-3.5 py-2.5 text-xs text-[#0A1628] hover:bg-[#FBF9F5] flex items-center gap-3 transition-colors group"
                    >
                      <div className="w-7 h-7 rounded-lg bg-[#C89B3C]/15 text-[#0D2440] flex items-center justify-center group-hover:bg-[#C89B3C] group-hover:text-[#0D2440] transition-colors flex-shrink-0">
                        <UserCheck className="w-4 h-4" />
                      </div>
                      <div className="min-w-0">
                        <div className="font-semibold text-[#0D2440] truncate">Faculty Login</div>
                        <div className="text-[11px] text-[#5C6B7A] truncate">Employee ID & Course Work</div>
                      </div>
                    </button>
                    ...
                    <button
                      ...
                      className="w-full text-left px-3.5 py-2.5 text-xs text-[#0A1628] hover:bg-[#FBF9F5] flex items-center gap-3 transition-colors group"
                    >
                      <div className="w-7 h-7 rounded-lg bg-[#2E7D5B]/15 text-[#2E7D5B] flex items-center justify-center group-hover:bg-[#2E7D5B] group-hover:text-white transition-colors flex-shrink-0">
                        <ShieldCheck className="w-4 h-4" />
                      </div>
                      <div className="min-w-0">
                        <div className="font-semibold text-[#0D2440] truncate">Admin Login</div>
                        <div className="text-[11px] text-[#5C6B7A] truncate">Controller of Exams & Office</div>
                      </div>
                    </button>
                    ...
                    >
                      <div className="w-7 h-7 rounded-lg bg-[#5C6B7A]/15 text-[#5C6B7A] flex items-center justify-center group-hover:bg-[#0D2440] group-hover:text-white transition-colors flex-shrink-0">
                        <Mail className="w-4 h-4" />
                      </div>
                      <div className="min-w-0">
                        <div className="font-semibold text-[#0D2440] truncate">Webmail Login</div>
                        <div className="text-[11px] text-[#5C6B7A] truncate">@becbgk.edu Institutional Mail</div>
                      </div>
                    </button>
                  </div>
                )}
            ...
            {/* Mobile Menu Hamburger Button (< 768px) */}
            <button
              id="mobile-nav-toggle-btn"
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="md:hidden p-1.5 sm:p-2 rounded-lg text-[#0D2440] hover:bg-[#0D2440]/10 transition-colors flex-shrink-0"
      ...
      {/* TIER 2: Dedicated Full-Width Horizontal Navigation Tabs Bar */}
      <div className="bg-[#0D2440] text-white border-b border-[#0D2440]/90 shadow-sm">
              ...
              return (
                ...
                >
                  <button
                    ...
                    className={`flex items-center gap-1 px-3 py-2 text-xs sm:text-[13px] font-medium rounded-md transition-all duration-150 whitespace-nowrap ${
                      isActive
                        ? 'text-[#C89B3C] bg-white/10 font-bold shadow-xs'
                        : 'text-slate-200 hover:text-white hover:bg-white/5'
                    }`}
                  >
                    <span>{item.label}</span>
                    {hasDropdown && (
                      <ChevronDown className={`w-3 h-3 transition-transform duration-200 opacity-70 ${isDropdownActive ? 'rotate-180 text-[#C89B3C]' : ''}`} />
                    )}
                  </button>

                  {/* Dropdown Menu - smart alignment to avoid viewport overflow */}
                  {hasDropdown && isDropdownActive && (
                    <div 
                      id={`dropdown-menu-${item.id}`}
                      className={`absolute ${idx >= 5 ? 'right-0' : 'left-0'} top-full mt-1 w-64 max-w-[calc(100vw-24px)] bg-white rounded-xl shadow-[0_12px_32px_-8px_rgba(10,22,40,0.25)] border border-[#E6E2D8] py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-150`}
                    >
      ...
      {mobileMenuOpen && (
        <div 
          ...
          className="md:hidden absolute inset-x-0 top-full bg-[#FBF9F5] border-b border-[#E6E2D8] shadow-2xl z-40 max-h-[calc(100vh-120px)] overflow-y-auto py-4 px-4 sm:px-6"
        >
